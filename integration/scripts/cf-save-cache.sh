#!/usr/bin/env bash
# cf-save-cache.sh — download a CloudFusion file from cloud into local cache.
...
notify() {
    if command -v notify-send &>/dev/null; then
        notify-send --icon=folder-cloud "CloudFusion" "$1"
...
for filepath in "$@"; do
    ...
    if [[ "$is_dir" == "True" ]]; then
        notify "Cannot cache a folder: $(basename "$filepath")"
        continue
    fi

    if [[ "$status" == "cached" ]]; then
        notify "Already in cache: $(basename "$filepath")"
        continue
    fi

    result="$(curl -sf -X POST "${API_BASE}/api/files/${file_id}/cache" 2>/dev/null)" || true
    if [[ -n "$result" ]]; then
        notify "Saved to cache: $(basename "$filepath")"
    else
        notify "Failed to save to cache: $(basename "$filepath")"
    fi
done
